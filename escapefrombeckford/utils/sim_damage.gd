...
static func compute_damage(battle: SimBattle, attacker_id: int, defender_id: int, base: int) -> int:
	...
	var dealt := battle.get_modified_value(attacker_id, base, Modifier.Type.DMG_DEALT)
	...
	var taken := battle.get_modified_value(defender_id, dealt, Modifier.Type.DMG_TAKEN)
