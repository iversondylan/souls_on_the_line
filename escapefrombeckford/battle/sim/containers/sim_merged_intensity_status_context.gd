...
var projected_stacks_bonus: int = 0

func _init(_owned_ctx: SimStatusContext = null, _projected_stacks_bonus: int = 0) -> void:
	owned_ctx = _owned_ctx
	projected_stacks_bonus = int(_projected_stacks_bonus)
...
func get_stacks() -> int:
	if owned_ctx == null:
		return projected_stacks_bonus
	return int(owned_ctx.get_stacks()) + projected_stacks_bonus
