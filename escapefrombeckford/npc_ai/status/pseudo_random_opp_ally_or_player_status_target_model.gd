# pseudo_random_opp_ally_or_player_status_target_model.gd
class_name PseudoRandomOppAllyOrPlayerStatusTargetModel extends ParamModel
...
func _write_target_ids(ctx: NPCAIContext) -> NPCAIContext:
	...
	elif player_id > 0 and int(ctx.api.get_group(player_id)) == opposing_group and ctx.api.is_alive(player_id):
		chosen_id = player_id
	else:
		var fallback_ids := ctx.api.get_combatants_in_group(opposing_group, false)
		chosen_id = int(fallback_ids[0]) if !fallback_ids.is_empty() else 0
